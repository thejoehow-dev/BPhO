...
for i in range (W):
    ...
    for j in range(0,int(N)):
        ...
        dx = s*math.sin(theta)*math.cos(alpha)
        dy = s*math.sin(theta)*math.sin(alpha)
        dz = s*math.cos(theta)
        x += dx
        y += dy
        z += dz
...
#ax.set_box_aspect((
#    max(all_x_points)-min(all_x_points),
#    max(all_y_points)-min(all_y_points),
#    max(all_z_points)-min(all_z_points)
#))

lims = min(min(all_x_points),min(all_y_points),min(all_z_points)), max(max(all_x_points),max(all_y_points),max(all_z_points))
...
if W == 1:
    ...
    t3 = " walks, step size "
ax.set_title("3D Random Walk, " + str(N) + t2 + str(W) + t3 + str(s))
