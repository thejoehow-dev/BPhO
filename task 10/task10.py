...
def toggle_m_animation():
    ...
    if animate_m:
        animate_m_button.configure(text="Stop m Animation")
        animate_m_slider()
    else:
...
def toggle_plane_animation():
    ...
    if plane_var.get() == "x-plane":
        ...
        if animate_x:
            animate_plane_button.configure(text="Stop x-plane Animation")
        ...
        if animate_y:
            animate_plane_button.configure(text="Stop y-plane Animation")
            animate_plane_slider()
        else:
        ...
        if animate_z:
            animate_plane_button.configure(text="Stop z-plane Animation")
            animate_plane_slider()
        else:
...
def update_contour_plot(*args):
    ...
    if contour_background_mode.get() == "Plane Slice":
        ...
        label_background = "Plane Normalised Probability Density - Background" if normalise_mode.get() == "Plane" else "Globally Normalised Probability Density - Background"
        fig.colorbar(background, ax=ax, label=label_background)
    elif contour_background_mode.get() == "White":
        ax.set_facecolor("white")
    elif contour_background_mode.get() == "Black":
...
controls.pack(side="left", fill="y", padx=15, pady=0)
ctk.CTkLabel(controls, text=f"Probability Densities\nof a Hydrogenic Atom", font=("Arial", 24, "bold")).pack(pady=(15, 15))
...
# m
ctk.CTkLabel(controls, text="Magnetic Quantum Number (m)").pack()
...
plane_frame.pack()
plane_menu = ctk.CTkOptionMenu(plane_frame, values=["x-plane","y-plane","z-plane"], variable=plane_var, width=100, command=plane_changed)
plane_menu.pack(pady=(0,5))
x_plane = 0
y_plane = 0
z_plane = 0
...
zero_plane_button = ctk.CTkButton(plane_frame, text="z = 0", command=zero_plane, width=70)
zero_plane_button.pack(pady=(0,15))
animate_m = False
m_animation_direction = 1
animate_x = False
x_animation_direction = 1
...
resol_interp_frame.pack(fill="x", padx=15, pady=(0,15))
...
normalise_frame.pack(fill="x", padx=15, pady=(0,15))
...
ctk.CTkButton(plots_row, text="Radial Graph", command=update_plot, width=90, height=35).pack(side="left", padx=(5,0), pady=(5,5))
ctk.CTkButton(plots_row, text="3D Plot", command=update_3D_plot, width=90, height=35).pack(side="right", padx=(0,5), pady=(5,5))
ctk.CTkButton(plots_row, text="Plane Slice", command=update_plane_slice, width=90, height=35).pack(pady=(5,5))
...
ctk.CTkOptionMenu(background_frame, values=["Plane Slice", "White", "Black"], variable=contour_background_mode, width=100, command=refresh_display).pack(side="right")
...
animate_row = ctk.CTkFrame(controls, fg_color="transparent")
animate_row.pack(fill="x", padx=15)
animate_m_button = ctk.CTkButton(animate_row, text="Animate m", command=toggle_m_animation, width=145, height=35)
animate_m_button.pack(side="left")
animate_plane_button = ctk.CTkButton(animate_row, text="Animate z-plane", command=toggle_plane_animation, width=145, height=35)
