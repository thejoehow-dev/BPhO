...
classical_label = ctk.CTkLabel(
    root,
    text="",
    font=("Arial", 20)
)
classical_label.pack(pady=5)
quantum_label = ctk.CTkLabel(
    root,
    text="",
    font=("Arial", 20)
)
...
def draw():
    ...
    angle1_label.configure(
        text=f"Angle 1: {angle1:.1f}°"
    )
    angle2_label.configure(
        text=f"Angle 2: {angle2:.1f}°"
    )
    a1 = math.radians(angle1)
    a2 = math.radians(angle2)
    classical = (
        1
        - (math.cos(a1)*math.cos(a2)) ** 2
        - (math.sin(a1)*math.sin(a2)) ** 2
    )
    quantum = (
        math.sin(math.radians(angle1 - angle2 + 360))
    ) ** 2
    classical_label.configure(
        text=f"Classical P(mismatch) = {classical:.6f}"
    )
    quantum_label.configure(
        text=f"Quantum P(mismatch) = {quantum:.6f}"
    )
